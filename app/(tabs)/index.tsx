...
function QuickAction({ emoji, title, subtitle, palette }: { emoji: string; title: string; subtitle: string; palette: PaletteKey }) {
  ...
  return (
    <Pressable
      style={{
        flex: 1, backgroundColor: bg, borderRadius: radii.card, padding: 16, gap: 12,
    ...
    >
      ...
      <View style={{ gap: 8 }}>
        <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
          <View style={{ gap: 3 }}>
            <Text style={{ fontSize: 15, color: colors.muted }}>{t("home.goodMorning")}</Text>
          ...
          <View
            style={{
              ...
              alignItems: "center", justifyContent: "center", boxShadow: shadows.brand,
            } as any}
          >
      ...
      <Card>
        ...
        <View style={{ flexDirection: "row", gap: 10, marginTop: 14 }}>
          <StatMini label={t("home.overview.burned")} value={`${todaySummary.caloriesBurned}`} unit={t("home.overview.kcalUnit")} palette="energy" />
          <StatMini label={t("home.overview.steps")}  value={`${(todaySummary.stepsWalked / 1000).toFixed(1)}k`} unit={t("home.overview.stepsUnit")} palette="sky" />
          <StatMini label={t("home.overview.active")} value={`${todaySummary.activeMinutes}`} unit={t("home.overview.minUnit")} palette="mint" />
        </View>
        <View style={{ gap: 6, marginTop: 18 }}>
          <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
            <Text style={{ fontSize: 12, color: colors.muted }}>{t("home.overview.goalLabel")}</Text>
          ...
          <Text style={{ fontSize: 11, color: colors.muted }}>
            {t("home.overview.goalProgress", { current: todaySummary.caloriesBurned, goal: todaySummary.caloriesGoal })}
          </Text>
        </View>
      </Card>
      ...
      {/* ── Quick Actions ── */}
      <View style={{ gap: 12 }}>
        <SectionHeader title={t("home.actions.title")} action={t("home.actions.seeAll")} />
        <View style={{ flexDirection: "row", gap: 12 }}>
          <QuickAction emoji="💪" title={t("home.actions.workout")} subtitle={t("home.actions.workoutSub")} palette="brand" />
          <QuickAction emoji="🥗" title={t("home.actions.workout")} subtitle={`${nutrition.calories.current} ${t("home.overview.kcalUnit")}`} palette="mint" />
        </View>
        <View style={{ flexDirection: "row", gap: 12 }}>
          <QuickAction emoji="📈" title={t("home.actions.progress")} subtitle={t("home.actions.progressSub")} palette="sky" />
          <QuickAction emoji="⭐" title={t("home.actions.coach")} subtitle={t("home.actions.coachSub")} palette="amber" />
        </View>
      </View>

      {/* ── Coach Tip ── */}
      <View
        style={{ backgroundColor: colors.brand, borderRadius: radii.card, padding: 20, gap: 10, borderCurve: "continuous" } as any}
      >
